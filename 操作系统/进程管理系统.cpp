...
void Queue::display() 
{
	...
	p=first;
	cout<<"**-------------------------------------------------------------------------**"<<endl;
    cout<<"进程名\t优先级\t运行时间  需要时间  状态  父进程  子进程   A  B  C"<<endl;
	while(p!=NULL)
	{
		cout<<"  "<<p->ID<<"\t  "<<p->Priority<<"\t  "<<p->CPUtime<<"\t  "<<p->Alltime<<"\t    "<<p->State<<"\t  "<<p->father<<"\t   "<<p->child<<"\t  ";
        for(int i=0;i<3;i++)     	
            	cout<<p->Allocation[i]<<"  ";
        cout<<endl;
		p=p->next;
